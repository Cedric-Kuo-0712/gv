...
BddNodeV::getLeftCofactor(unsigned i) const {
    ...
    BDD_EDGEV_FLAG f = (isNegEdge() ^ t.isNegEdge()) ? BDD_NEG_EDGEV : BDD_POS_EDGEV;
    if (t.isNegEdge()) {
        t = ~t;
        e = ~e;
    }
...
bool BddNodeV::operator<(const BddNodeV& n) const {
...
bool BddNodeV::isTerminal() const {
...
BddNodeV::nodeMoveRecur(unsigned fromLevel, unsigned toLevel, map<size_t, size_t>& moveMap) const {
    ...
    BddNodeVInt* n = _BddMgrV->uniquify(left(), right(), thisLevel - fromLevel + toLevel);
...
// return true if any
//
bool BddNodeV::containNode(unsigned bLevel, unsigned eLevel) const {
...
bool BddNodeV::getCubeRecur(bool phase, size_t& ith, size_t target, BddNodeV& res) const {
    if (isTerminal()) {
        if (!(phase ^ isNegEdge())) {
...
void BddNodeV::getAllCubesRecur(bool phase, BddNodeV& cube, vector<BddNodeV>& allCubes) const {
    if (isTerminal()) {
        if (!(phase ^ isNegEdge()))
...
bool BddNodeV::toStringRecur(bool phase, string& str) const {
    if (isTerminal())
...
operator<<(ostream& os, const BddNodeV& n) {
    ...
    os << endl
       << endl
       << "==> Total #BddNodeVs : " << nNodes << endl;
    return os;
}

void BddNodeV::print(ostream& os, size_t indent, size_t& nNodes) const {
...
void BddNodeV::unsetVisitedRecur() const {
    BddNodeVInt* n = getBddNodeVInt();
...
void BddNodeV::drawBdd(const string& name, ofstream& ofile) const {
    ofile << "digraph {" << endl;
...
void BddNodeV::drawBddRecur(ofstream& ofile) const {
    BddNodeVInt* n = getBddNodeVInt();
    if (n->isVisited())
        return;
    n->setVisited();
